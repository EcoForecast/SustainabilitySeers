...
nee.df <- nee.df %>% dplyr::filter(site_id == "HARV", 
...
# data$Precip <- data$Precip - mean(data$Precip)
# data$Temp <- data$Temp - mean(data$Temp)
# data$SW <- data$SW - mean(data$SW)

#normalize
# data$Precip <- (data$Precip-min(data$Precip))/(max(data$Precip)-min(data$Precip))*100
